...
SysY 语言是编译技术实验所完成的编译器的源语言，是 C 语言的一个子集。一个 SysY 语言源程序文件中有且仅有一个名为 `main` 的主函数定义，除此之外包含若干全局变量声明、常量声明和其他函数定义。SysY 语言支持 32 位有符号数 `int` 类型、8 位有符号数 `char` 类型及其的一维数组类型；`const` 修饰符用于声明常量。

SysY 语言提供了输入/输出 (I/O) 的语言构造（`getint`、`getchar`、`printf`），它们在 SysY 程序中的函数内作为语句使用。

- **函数**：函数可以带参数也可以不带参数，参数的类型可以是 `int`、`char` 或其一维数组类型；函数可以返回 `int` 或 `char` 类型的值，或者不返回值（即声明为 `void` 类型）。当参数为 `int` 或 `char` 时，按值传递；而参数为数组类型时，实际传递的是数组的起始地址。函数体由若干变量声明和语句组成。
- **变量/常量声明**：可以在一个变量/常量声明语句中声明多个变量或常量，声明时可以带初始化表达式。所有变量/常量要求先定义再使用。在函数外声明的为全局变量/常量，在函数内声明的为局部变量/常量。
- **语句**：语句包括赋值语句、表达式语句（表达式可以为空）、语句块、`if` 语句、`for` 语句、`break` 语句、`continue` 语句、`return` 语句、以及特定的 I/O 语句。语句块中可以包含若干变量声明和语句。
- **表达式**：支持基本的算术运算 (`+`、`-`、`*`、`/`、`%`)、关系运算 (`==`、`!=`、`<`、`>`、`<=`、`>=`) 和逻辑运算 (`!`、`&&`、`||`)，非 0 表示真、0 表示假，而关系运算或逻辑运算的结果用 1 表示真、0 表示假。算符的优先级和结合性以及计算规则（含逻辑运算的“短路计算”）与 C 语言一致。

## 二、运行时库与 I/O

源程序可通过 `getint`、`getchar` 与 `printf` 完成 I/O 交互。

如果希望在 C 语言中测试测试程序，只需要将 `getint` 等加入函数声明即可，示例如下：
...
`printf` 的使用方法为 `printf` `(` `<StringConst>` {`,` `<Exp>`} `)`，其中 `StringConst` 为字符串常量终结符。
...
- 终结符或者是由单引号括起的串，或者是 `Ident`、`IntConst`、`StringConst`、`CharConst` 这样的记号
...
变量定义 VarDef       → Ident [ '[' ConstExp ']' ]
                      | Ident [ '[' ConstExp ']' ] '=' InitVal // 包含普通变量、一维数组定义
...
语句 Stmt             → LVal '=' Exp ';' // 每种类型的语句都要覆盖
                      ...
                      | 'for' '(' [ForStmt] ';' [Cond] ';' [ForStmt] ')' Stmt 
                        // 1.无缺省 2.ForStmt与Cond中缺省一个 3.ForStmt与Cond中缺省两个 4.全部缺省
                      | 'break' ';'
                      | 'continue' ';'
...
语句 ForStmt          → LVal '=' Exp // 1.花括号内重复0次 2.花括号内重复多次
...
常量表达式 ConstExp   → AddExp // 注：使用的Ident必须是常量，存在即可
```
...
其中，`identifier-nondigit` 为下划线或大小写字母，`digit` 为 0 到 9 的数字。

**注**：请参考 ISO/IEC 9899 第 51 页关于标识符的定义。
...
**注**：请参考 ISO/IEC 9899 第 66 页关于注释的定义。
...
**注**：请参考 ISO/IEC 9899 第 54 页关于整型常量的定义，在此基础上**忽略所有后缀**。

**(4) 字符常量**

SysY 语言中字符常量 `CharConst` 的定义如下：

```pseudocode
<CChar>       → 十进制编码为 32-38,40-126 的 ASCII 字符，'\' (编码92) 出现当且仅当为 '\n'
<CharConst>   → "'" <CChar> "'"
```
...
```pseudocode
<FormatChar>  → %d | %c
<NormalChar>  → 十进制编码为 32,33,35-126 的 ASCII 字符，'\' (编码92) 出现当且仅当为 '\n'
...
1. 为了降低开发难度，保证所有测试程序中 `getint`、`getchar` 与 `printf` 只会作为 I/O 语句出现，不会作为自定义的变量/函数标识符。
2. `printf` 函数默认不换行。
...
1. `ConstDef` 用于定义符号常量。`ConstDef` 中的 `Ident` 为常量的标识符，在 `Ident` 后、`=` 之前是可选的一维数组以及一维数组长度的定义部分，在之后是初始值。常量必须有对应的初始值。
...
2. 任何变量或常量的初值（包括数组元素）如果不匹配其声明类型，需进行隐式类型转换（见下文）。
...
6. `StringConst` 仅能用于初始化 `char` 类型的数组。

**类型转换 (Type Conversion)**

SysY 语言支持 `int` 类型与 `char` 类型之间的隐式转换：

1. **算术运算提升**：在表达式计算中，`char` 类型的操作数会被隐式转换为 `int` 类型参与运算。
2. **赋值转换**：
   - 将 `int` 类型赋值给 `char` 类型变量时，截取低 8 位。
   - 将 `char` 类型赋值给 `int` 类型变量时，进行符号扩展。
3. **参数传递**：函数调用时，若形参与实参类型不一致（仅限 `int` 与 `char` 之间），按照赋值转换规则处理。
...
1. `Exp` 在 SysY 中代表 `int`、`char` 或 `void` 型的表达式；`Cond` 代表条件表达式，故它定义为 `LOrExp`。前者的单目运算符中不出现 `!`，后者可以出现。
...
1. `ConstExp` 在 SysY 中代表 `int` 或 `char` 型表达式。
...
错误类别 `a` 为词法分析中会出现的错误。 错误类别 `i`, `j`, `k` 为语法分析中会出现的错误。 剩余错误类别均为语义分析中会出现的错误。

| 错误类别码 | 错误类型                                | 解释                                                         | 对应文法及出错符号，... 表示省略该条规则其他部分             |
| ---------- | --------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| a          | 非法符号                                | 出现了 `&` 和 `|` 这两个符号，应该将其当做 `&&` 与 `||` 进行处理，但是在记录单词名称的时候仍记录 `&` 和 `|`，报错行号为 `&` 或 `|` 所在的行号。 | `LAndExp → LAndExp '&&' EqExp` `LOrExp → LOrExp '||' LAndExp` |
| b          | 名字重定义                              | 函数名或者变量名在当前作用域下重复定义。注意，变量一定是同一级作用域下才会判定出错，不同级作用域下，内层会覆盖外层定义。报错行号 `Ident` 所在行数。 | `ConstDef → Ident ...` `VarDef → Ident ...` `Ident ...` `FuncDef → FuncType Ident ...` `FuncFParam → BType Ident ...` |
| c          | 未定义的名字                            | 使用了未定义的标识符。报错行号为 `Ident` 所在行数。          | `LVal → Ident ...` `UnaryExp → Ident ...`                    |
| d          | 函数参数个数不匹配                      | 函数调用语句中，参数个数与函数定义中的参数个数不匹配。报错行号为函数调用语句的函数名所在行数。 | `UnaryExp → Ident '(' [FuncRParams] ')'`                     |
| e          | 函数参数类型不匹配                      | 函数调用语句中，参数类型与函数定义中对应位置的参数类型不匹配。报错行号为函数调用语句的函数名所在行数。关于参数类型不匹配会在下方特别说明进行详细说明。 | `UnaryExp → Ident '(' [FuncRParams] ')'`                     |
| f          | 无返回值的函数存在不匹配的 return 语句  | 报错行号为 `return` 所在行号。                               | `Stmt → 'return' { '[' Exp ']' } ';'`                        |
| g          | 有返回值的函数缺少 return 语句          | 只需要考虑函数末尾是否存在 return 语句，无需考虑控制流。报错行号为函数结尾的 `}` 所在行号。 | `FuncDef → FuncType Ident '(' [FuncFParams] ')' Block` `MainFuncDef → 'int' 'main' '(' ')' Block` |
| h          | 不能改变常量的值                        | `LVal` 为常量时，不能对其修改。报错行号为 `LVal` 所在行号。  | `Stmt → LVal '=' Exp ';'` `ForStmt → LVal '=' Exp`           |
| i          | 缺少分号                                | 报错行号为分号前一个非终结符所在行号。                       | `Stmt`, `ConstDecl` 及 `VarDecl` 中的 `;`                    |
| j          | 缺少右小括号 `)`                        | 报错行号为右小括号前一个非终结符所在行号。                   | 函数调用 (`UnaryExp`)、函数定义 (`FuncDef`, `MainFuncDef`)、`Stmt` 及 `PrimaryExp` 中的 `)` |
| k          | 缺少右中括号 `]`                        | 报错行号为右中括号前一个非终结符所在行号。                   | 数组定义 (`ConstDef`, `VarDef`, `FuncFParam`) 和使用 (`LVal`) 中的 `]` |
| l          | printf中格式字符与表达式个数不匹配      | 报错行号为 `printf` 所在行号。                               | `Stmt → 'printf' '(' StringConst {',' Exp} ')' ';'`          |
| m          | 在非循环块中使用 break 和 continue 语句 | 报错行号为 `break` 与 `continue` 所在行号。                  | `Stmt → 'break' ';'`                                         |

### 错误输出示例

**输出结构** `错误的行号 错误的类别码` (中间仅用一个空格间隔)
