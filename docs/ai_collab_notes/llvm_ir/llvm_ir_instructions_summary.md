...
**与 LLVM 20 的对应**：格式以 LLVM 20.1.0 LangRef 与不透明指针（opaque pointer，`ptr`）为准；若评测环境为 LLVM 12，指针类型可能需使用 `i32*`/`i8*` 等。
...
- **格式**：
  ...
  - `declare void @putstr(ptr)`（LLVM 15+ 不透明指针下用 `ptr`；若评测为 LLVM 12 可写 `i8*`）
  未用到的可省略。
...
- **格式**：`define dso_local 返回类型 @函数名(形参列表) #属性 { 函数体 }`
  ...
  - 形参：普通参数为 `i32 %0`、`i8 %1` 等；数组形参为 `ptr` 类型（LLVM 15+ 默认不透明指针；旧版可用 `i32*`）。
...
- **生成时机（文法/AST）**：
  ...
  - `printf '(' StringConst { ',' Exp } ')' ';'` → 根据格式串与 Exp 列表，拆成多次 `call void @putint(i32)` / `call void @putch(i32)` / `call void @putstr(ptr)`（或等价的声明），并保证格式字符与表达式个数匹配。
...
- **评测**：课程评测可能为 LLVM 12.0.0；若使用 LLVM 15+（含 LLVM 20），请采用不透明指针 `ptr` 及上述格式；需在 IR 头部声明用到的 getint/getchar/putint/putch/putstr。
